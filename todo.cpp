...
int del(int k){
  ...
  if(k > v.size() || k < 1){
    //if the task number entered is greater than the number of task , then it cannot be elected ot it does not exist
...
int done(int k){
  // we will be printing exact time of the completion also
  ...
  // these are just time function, using the current date
  ...
  if(k > v.size()){
    ...
    std::ofstream write1, write2;
    write1.open("todo.txt");
    write2.open("todo.txt", ios::app);
    for(int i = 0; i < v.size(); i++){
      if(i == k - 1){
        x = v[i];
...
int help(){//help
  //this is main , without it  you cant even run your app, its liek a user manual
...
int report(){
  ...
  std::ifstream read; // todo.txt contains the pending tasks
  read.open("todo.txt");
  if(read!=NULL){
    while(getline(read,s)){
      pending++;
...
int main(int argc, char** argv){
  ...
  if(argc == 1 || s == "help"){
    ...
    //1- if done is called and the task number which completed is not mentioned
    //2- if task number is mentioned
